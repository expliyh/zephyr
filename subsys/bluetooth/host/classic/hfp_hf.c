			 ...
			 enum at_cme cme_err)
{
	...
	if (bt_hf && bt_hf->unsolicited_complete) {
		bt_hf->unsolicited_complete(hf, err);
	} else if (bt_hf && bt_hf->unsolicited) {
		bt_hf->unsolicited(hf, NULL, NULL);
	}
...
/*
 * Check whether @p cmd matches a known built-in HFP AT command.
 * @param cmd     AT command string (must start with "AT").
 * @param cmd_len Length of @p cmd (excluding trailing \r / \n).
 * @return true if @p cmd is a known built-in command.
 */
#define HFP_KNOWN_CMD_ENTRY(s) { .name = s, .len = sizeof(s) - 1U }

static bool hfp_hf_is_known_cmd(const char *cmd, size_t cmd_len)
{
	static const struct {
		const char *name;
		uint8_t len;
...
int bt_hfp_hf_send_at(struct bt_hfp_hf *hf, const char *cmd)
{
	...
	if (hfp_hf_is_known_cmd(cmd, cmd_len)) {
...
static int hfp_hf_unsolicited_handle(struct at_client *hf_at,
				     struct net_buf *buf)
{
	struct bt_hfp_hf *hf = CONTAINER_OF(hf_at, struct bt_hfp_hf, at);
	char *cmd = NULL;
	...
	if (hf_at->rsp_buf.len > 0U) {
		...
		if (hf_at->rsp_buf.len < hf_at->rsp_buf.size) {
			cmd[hf_at->rsp_buf.len] = '\0';
		}
	}

	if (bt_hf && bt_hf->unsolicited) {
		bt_hf->unsolicited(hf, cmd, data);
	}
	...
	if (err < 0) {
		LOG_ERR("unsolicited_handle: expected NUL byte not found");
		return err;
	}

	err = at_check_byte(buf, '\n');
	if (err < 0) {
		LOG_ERR("unsolicited_handle: expected LF byte not found");
		return err;
	}
...
int unsolicited_cb(struct at_client *hf_at, struct net_buf *buf)
{
	...
	if (!handler) {
		if (bt_hf && bt_hf->unsolicited) {
			return hfp_hf_unsolicited_handle(hf_at, buf);
		}
